...
# 获取百度百科页面
def getBikePage(word, id):
    if id != None:
        res = request.urlopen("https://baike.baidu.com/item/" + parse.quote(word) + "/" + id)
    else:
...
def parseContent(bikeword, id=None):
    titleSet = set()
    html = getBikePage(bikeword, id)
